...
class ServicePolicyServiceTest {
    ...
    private ServicePolicyService.AcceptRequest request() {
        return new ServicePolicyService.AcceptRequest(ServicePolicyService.TERMS_VERSION, ServicePolicyService.PRIVACY_VERSION, true, true, true);
    }
    ...
    @Test void previouslyAcceptedMissingBirthdayIsBlockedOnEveryNewRequest() {
        when(records.findById(7L)).thenReturn(Optional.of(new ServicePolicyAcceptance(user,
                ServicePolicyService.TERMS_VERSION, ServicePolicyService.PRIVACY_VERSION, OffsetDateTime.now(clock))));
        assertThat(service.status(7L).accepted()).isFalse();
        assertCode(() -> service.requireEligible(7L), ErrorCode.AGE_INFORMATION_REQUIRED);
        birthday("2000-01-01");
    ...
    @Test void staleVersionAndMissingOrFalseConfirmationDoNotWrite() {
        assertCode(() -> service.accept(7L, new ServicePolicyService.AcceptRequest("old", ServicePolicyService.PRIVACY_VERSION, true, true, true)), ErrorCode.POLICY_VERSION_MISMATCH);
        assertCode(() -> service.accept(7L, new ServicePolicyService.AcceptRequest(ServicePolicyService.TERMS_VERSION, ServicePolicyService.PRIVACY_VERSION, null, true, true)), ErrorCode.POLICY_ACCEPTANCE_INVALID);
        assertCode(() -> service.accept(7L, new ServicePolicyService.AcceptRequest(ServicePolicyService.TERMS_VERSION, ServicePolicyService.PRIVACY_VERSION, true, false, true)), ErrorCode.POLICY_ACCEPTANCE_INVALID);
        assertCode(() -> service.accept(7L, new ServicePolicyService.AcceptRequest(ServicePolicyService.TERMS_VERSION, ServicePolicyService.PRIVACY_VERSION, true, true, false)), ErrorCode.POLICY_ACCEPTANCE_INVALID);
        verify(records, never()).save(any());
    }
    @Test void repeatedAcceptancePreservesOriginalTimestampAndRequiresAccountLock() {
        ...
        when(records.findById(7L)).thenReturn(Optional.of(new ServicePolicyAcceptance(user, ServicePolicyService.TERMS_VERSION, ServicePolicyService.PRIVACY_VERSION, original)));
