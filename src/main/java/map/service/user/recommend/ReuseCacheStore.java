 ...
 * - recommend:cache-hits:{hash} : 히트 횟수 카운터(TTL cacheTtl)
 *
...
public class ReuseCacheStore {
    ...
    /**
     * jobId 에 연결된 hash 를 조회하고(GET) 즉시 삭제한다(DEL, 1회성 연결고리).
     * 연결이 없으면 Optional.empty.
     */
    public Optional<String> consumeLink(String jobId) {
        String key = linkKey(jobId);
    ...
    /**
     * hash 의 히트 카운터를 1 증가(INCR)시키고 TTL 을 cacheTtl 로 갱신한 뒤,
     * 증가 후 값을 반환한다. Redis 응답이 null 이면 0 을 반환한다.
     */
